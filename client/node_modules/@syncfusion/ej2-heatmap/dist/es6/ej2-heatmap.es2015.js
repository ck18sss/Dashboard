    ...
    getMaxLabelSize(axis, heatmap) {
        ...
        for (let i = 0; i < labels.length; i++) {
            ...
            if (axis.textStyle.textOverflow === 'Wrap') {
                ...
                const wrappedlabels = textWrap(label, labelInterval, axis.textStyle);
    ...
    calculateCategoryAxisLabels() {
        ...
        if (labels && labels.length > 0) {
            for (let i = min; i <= max; i = i + interval) {
                const value = !isNullOrUndefined(labels[i]) ? labels[i].toString() : i.toString();
                this.axisLabels.push(value);
            }
        }
        else {
...
class AxisHelper {
    ...
    drawXAxisLabels(axis, parent, rect) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            let label = (axis.textStyle.textOverflow !== 'Wrap' && axis.textStyle.textOverflow !== 'Trim' && axis.labelIntersectAction === 'Trim' && axis.isIntersect) ? axis.valueType !== 'DateTime' ||
            ...
            if (axis.textStyle.textOverflow === 'Wrap') {
                const wrappedlabels = textWrap(wrappedLabel, interval - (axis.border.width / 2), axis.textStyle);
                let labelWidth = 0;
                let wrappedlabelSize = new Size(0, 0);
                for (let index = 0; index < wrappedlabels.length; index++) {
                    wrappedlabelSize = measureText(wrappedlabels[index], axis.textStyle);
            ...
            if (angle % 360 !== 0) {
                ...
                const rotateSize = rotateTextSize(axis.textStyle, wrappedLabel, angle);
    ...
    drawYAxisLabels(axis, parent, rect) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            let labelLength = 1;
            if (axis.textStyle.textOverflow === 'Wrap') {
                const wrappedLabel = textWrap(label, axis.maxLabelLength, axis.textStyle);
            ...
            const x = labelRect.x + padding + (axis.opposedPosition ? (axis.textStyle.textOverflow === 'Wrap' && axis.angle % 360 !== 0 ? labelLength * (elementSize.height / 2) : 0) + axisWidth : -axisWidth);
            ...
            const y = (labelRect.y - indexValue) + position - ((labelLength > 1 && axis.textStyle.textAlignment === 'Center') || (axis.angle !== 0 && axis.opposedPosition) ? (((elementSize.height * labelLength) / 2) - (elementSize.height / 2)) : 0);
            const options = new TextOption(heatMap.element.id + '_YAxis_Label' + i, new TextBasic(x, y, anchor, label, 0, 'rotate(' + axis.angle + ',' + (x) + ',' + (y) + ')', 'middle'), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
...
let HeatMap = class HeatMap extends Component {
    ...
    render() {
        ...
        if (this.theme === 'Bootstrap5' || this.theme === 'Bootstrap5Dark') {
            const textSettings = { title: { textStyle: { size: '12px', fontFamily: 'system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", "Liberation Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"', fontWeight: '500' } }, textStyle: { size: '12px', fontFamily: 'system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", "Liberation Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"' } };
